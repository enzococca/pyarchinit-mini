...
# -*- coding: utf-8 -*-
"""
Test SFDP GraphML Export for Large Graph (760 US) - REVERSE EPOCHS
...
def test_sfdp_graphml_export_reverse():
    """Test GraphML export with sfdp for Dom zu Lund (760 US) - REVERSE EPOCHS"""

    print("=" * 80)
    print("SFDP GraphML Export Performance Test - REVERSE EPOCHS")
    print("=" * 80)
    print()
    ...
    output_path = "/Users/enzo/Desktop/dom_zu_lund_harris_matrix_REVERSE.graphml"

    print(f"📊 Testing site: {site_name}")
    print(f"📁 Output file: {output_path}")
    print(f"🔄 Reverse epochs: YES")
    print()
    ...
    # Step 2: Export to GraphML with REVERSE EPOCHS
    print("Step 2: Exporting to GraphML with REVERSE epochs...")
    print("   (Epochs will be ordered from newest to oldest)")
    print()
    ...
    try:
        result_path = matrix_generator.export_to_graphml(
            ...
            site_name=site_name,
            title="Dom zu Lund - Harris Matrix (REVERSE epochs)",
            reverse_epochs=True  # ← REVERSE EPOCHS ENABLED
        )
        ...
        if result_path and os.path.exists(result_path):
            ...
            print(f"Nodes:                  {num_nodes}")
            print(f"Edges:                  {num_edges}")
            print(f"Epoch order:            REVERSED (newest → oldest)")
            print()

            # Determine if sfdp was used
            if num_nodes > 700 or num_edges > 2000:
                print("⚠️  Very large graph - layout pre-calculation SKIPPED")
                print("   yEd will calculate the layout when you open the file")
            elif num_nodes > 500 or num_edges > 1500:
            ...
            print("3. Verify epochs are ordered from NEWEST to OLDEST")
...
if __name__ == "__main__":
    test_sfdp_graphml_export_reverse()